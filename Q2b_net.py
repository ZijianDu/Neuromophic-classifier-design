...
class Round3(UnaryScalarOp):
    ...
    def c_code(self, node, name, (x,), (z,), sub):
        #return "%(z)s = floor(%(x)s*4)/3;" % locals()
        return "%(z)s = (round(1.5*(%(x)s+1))/1.5)-1;" % locals()
...
def binarization(W,H,binary=True,deterministic=False,stochastic=False,srng=None):
    ...
    if not binary or (deterministic and stochastic):
        ...
        if stochastic:
            print('Warning: stochastic is disabled during weight quantization changing to deterministic')
            Wb = T.floor(Wb * 4) / 3
...
def train(train_fn,val_fn,
    ...
    for epoch in range(num_epochs):
        ...
        err_measure.append([epoch, train_loss, val_loss, val_err, test_loss, test_err])
    ...
    print("  test loss:                     " + str(test_loss))
    print("  test error rate:               " + str(test_err) + "%")
    return [err_measure, lasagne.layers.get_all_param_values(model)]